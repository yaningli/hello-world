...
# here we can see three different types of data stored in variables: an integer, 
# a string, and a float. You do not have to declare the data type stored in each 
# variable. Python does that for you. You can see what type of data is in a variable 
...
# MATH FUNCTIONS
# ------------------------------------------------
# built-in arithmetic functions are: add, subtract, multiply, divide, power, 
# integer division, and modulus (AKA: mod or remainder)
#	+  Addition
# 	-  Subtraction
# 	*  Multiplication
# 	/  Division
...
def console_input():
    ...
    # With what we know so far we can write a program to get user input and 
...
# all methods have the self parameter, which is Python's reference to the object 
...
def file_io():
    ...
    # opens a file handle called fin, iterates and prints each line
    with open(filename) as fin:
        for line in fin:
            print(line)

    # grab words from a line by using split, turning each line into a list row
    with open(filename) as fin:
        fin.readline()
        for line in fin:
            row = line.split(',')
...
# Python Language Features and Technics
# ------------------------------------------------
def lambda_funcs():
    '''
    General syntax of a Python lambda is: 
            lambda arguments: expression
    Note:
    Lambda functions can accept zero or more arguments but only one expression.
    The return value of the lambda function is the value that this expression is
    evaluated to. 
    '''
    ...
    print("\nlambda function wiht multiple argument")
    f2 = lambda x, y, z: x*x*x + y*y*y + z*z*z + x*y*z
    print(f2(1,2,3))

    print("\nlambda function with multiple line")
    print("No, we cann't !!! Python lambda functions accept only one expression")

    print("\nusing lambdas with map")
    L0 = [1, 2, 3, 4]
    print(L0)
    L0 = list(map(lambda x: x**2, L0))    # square each element of the list using map
    print(L0)

    print("\nusing lambdas using filter function") 
    L1 = [1, 3, 2, 5, 20, 21]
    print(L1)
    L2 = list(filter(lambda x: x%2 == 0, L1))
    print(L2)

    print("\nusing lambdas with list sortin")
    dic = {"Alex":20, "Bob":14, "Lucas":2, "Tony":5, "Connor":36, "Lyn":33}
    print(dic)
